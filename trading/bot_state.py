        ...
        if not isinstance(status, dict) or not status.get('pending'):
            if isinstance(status, dict) and status.get('reconciled'):
                ...
                enriched.update({
                    ...
                    'reconciliation_source': status.get('reconciliation_source'),
                    'reconciled_at': status.get('reconciled_at'),
                    'reconciliation_observed_at': status.get('reconciliation_observed_at'),
                    'previous_pending': status.get('previous_pending'),
