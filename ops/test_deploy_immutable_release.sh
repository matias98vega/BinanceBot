...
test_spot_safety_contracts() {
  ...
  )" || fail 'S1-S22/C-prev1-C-prev2/C1-C10/G17-P1-P9/G17-N1-N22/G17-R1-R18 Python Spot safety fixtures failed'
  printf '%s\n' "$output"
  python_pass_count="$(grep -c '^\[PASS\] ' <<< "$output")"
  [[ "$python_pass_count" -eq 83 ]] || fail "unexpected Spot Python assertion count: ${python_pass_count}"
